...
const I18N = {
  pl: {
    ...
    helpMetric: 'Średnia to zwykły wynik procentowy — tyle procent punktów zdobyli przeciętnie uczniowie tej szkoły. Wynik znormalizowany ustawia szkołę względem średniej województwa z tego samego roku, więc nie przeszkadza mu, że jeden egzamin był łatwiejszy od drugiego. Kliknij, aby przeczytać o wszystkich metrykach.',
  ...
  en: {
    ...
    helpMetric: 'Mean is the plain percentage score — the share of points this school\'s pupils scored on average. Normalised score places the school against the voivodeship average from the same year, so it is not thrown off by one exam being easier than another. Click to read about all the metrics.',
